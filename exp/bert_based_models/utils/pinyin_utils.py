...
from bbcm.utils.basic_utils import edit_distance
...
class Py2HzUtils(object):
    ...
    def to_hanzi(self, pinyin_tuple, method='viterbi join', n_candidates=None, with_rank=False):
        ...
        results = self._py2hz_algorithm(
            ...
            method=method)
        if with_rank:
            return [(item.score, ''.join(item.path)) for item in results]
        if 'join' in method:
            results = list(map(
...
def is_pinyin_similar_char(err_char, cor_char, heteronym=False):

    def _is_similar_pinyin_str(_err_py, _cor_py):
        if False:  # 是否允许平卷舌不分
            _err_py.replace('ng', 'n')
            _cor_py.replace('ng', 'n')
        if False:  # 是否允许前后鼻音不分
            _err_py.replace('zh', 'z').replace('ch', 'c').replace('sh', 's')
            _cor_py.replace('zh', 'z').replace('ch', 'c').replace('sh', 's')
        if False:  # 是否允许 f/h n/l 不分
            _err_py.replace('f', 'h').replace('n', 'l')
            _cor_py.replace('f', 'h').replace('n', 'l')
        # 假如首字母不同
        if _err_py[0] != _cor_py[0]:  
            # 两者不为 "仅多一个首字母" 的关系：奥->好
            if (not _err_py in _cor_py[1:]) and (not _cor_py in _err_py[1:]): 
                # 允许首字母打错，但后面必须完全相同：变->骗
                if _err_py[1:] != _cor_py[1:]:
                    return False
        # 整个拼音里不能有多于 2 个编辑错误
        if edit_distance(_err_py, _cor_py, rate=False) > 2:
            return False
        # 整个拼音里不能有 67% 以上的编辑错误
        if edit_distance(_err_py, _cor_py, rate=True) > 0.67:
            return False
        return True

    if heteronym:
        # ['dan', 'shan']
        err_py_case = pypinyin.pinyin(
            err_char, heteronym=True, style=pypinyin.NORMAL,
            errors=lambda _x: '[PAD]' if _x == '[PAD]' else '[UNK]')[0]
    else:
        # ['dan']
        err_py_case = pypinyin.lazy_pinyin(
            cor_char, errors=lambda _x: '[PAD]' if _x == '[PAD]' else '[UNK]')[0]
    
    cor_py = pypinyin.lazy_pinyin(
        cor_char, errors=lambda _x: '[PAD]' if _x == '[PAD]' else '[UNK]')[0]
    
    # 错字的多音字中任何一个和正确字相近就行
    for err_py in err_py_case:  
        if _is_similar_pinyin_str(err_py, cor_py):
            return True
    return False    
    

def is_pinyin_similar_word(word_a, word_b, heteronym=False):
    if word_a.startswith('##'):
        word_a = word_a[2:]
    if word_b.startswith('##'):
        word_b = word_b[2:]
    if len(word_a) == len(word_b) == 1:
        return is_pinyin_similar_char(
            word_a, word_b, heteronym=heteronym)

    # 成词拥有上下文确定读音，可不考虑多音字
    o_py = pypinyin.lazy_pinyin(
        word_a, errors=lambda _x: '[PAD]' if _x == '[PAD]' else '[UNK]')
    c_py = pypinyin.lazy_pinyin(
        word_b, errors=lambda _x: '[PAD]' if _x == '[PAD]' else '[UNK]')

    ed_value = edit_distance(''.join(o_py), ''.join(c_py), rate=False)
    ed_value_char = [edit_distance(_o, _c, rate=True) 
                     for _o, _c in zip(o_py, c_py)]
    # 前都对，尾只对首字母： 超短裙->超短期
    if set(ed_value_char[:-1]) == set([1]) and o_py[-1][0] == c_py[-1][0]:
        return True
    # 整个拼音里不能有多于 2 个编辑错误
    if ed_value > 2:
        return False
    # 单个拼音里不能有 67% 以上的编辑错误
    if max(ed_value_char) > 0.67:
        return False
    return True
